...
[
  ...
  [ 'format'=>'raw',
            ...
            'value'=>function($data){
                  $ids=$data->id;
          $model =Assetspathology::find()->where(['id'=>$ids])->one();
          $idd=$model->id;
...
[
      ...
      $var =app\models\Cartassets::find()->where(['id'=>$data->id])->one();
      $id=$var->id;
if($var->dep=="Pathology"&&$var->returnables=='Y'){
...
var personnelid=$("#assetspathology-personnelid").val();
//var quantity=$("#assetspathology-quantity").val();
var office=$("#assetspathology-office").val();
var returnables=$("#assetspathology-returnables").val();
var doreturnable=$("#assetspathology-doreturnable").val();
...
var href="<?=Yii::$app->request->baseUrl;?>/index.php/assetspathology/create2/"+"?id="+id+"&personnelid="+personnelid+"&office="+office+"&returnables="+returnables+"&doreturnable="+doreturnable;
        window.location.href=href;
          }}
    function returndata()
       {
var id=$("#assetsid").val();
var href="<?=Yii::$app->request->baseUrl;?>/index.php/assetspathology/create3/"+"?id="+id;
        window.location.href=href;
          }
</script> 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <?php
     $office=['Admin'=>'CHAMPS','Microlab'=>'Microlab','Clinical'=>'Clinical','IT'=>'IT(Information Technology)','SBS'=>'SBS','Pathology'=>'Pathology'];
...
function getRelatedFields(personnelid){
    $.ajax({
        ...
        success:function (data) {
             $('#assetspathology-fname').val(data.fname);
             $('#assetspathology-lname').val(data.lname);
             $('#assetspathology-personnelid').val(data.personnelid);
             $('#assetspathology-office').val(data.office);
        },
