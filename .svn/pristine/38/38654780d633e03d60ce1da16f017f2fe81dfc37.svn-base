...
/**
 * AssetsmicroSearch represents the model behind the search form of `app\models\Assetsmicro`.
 */
class AssetsmicroSearch extends Assetsmicro
{
    /**
     * {@inheritdoc}
     */
 ...
 public function search2($params,$id)
    {
        $query = Assetsmicro::find()->where(['id'=>$id]);
        // add conditions that should always apply here
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);
