    ...
    'columns'=>[
        [
          ...
          'value'=>function ($data) {
                ...
                $last=substr($dicom,-6);
//if($req->dep=="Microlab"){
  if($req->department==Yii::$app->user->identity->Type){
...
//return Html::a('Approved', ['request/lview','id'=>$id],['class'=>'btn btn-xs btn-success glyphicon glyphicon-ok']);
 $btn='<a style="background-color:blue" data-method="post" data-confirm="Are you sure you want to approve?" href="'.Url::to(["request/lview",'id'=>$id]).'"><i class="btn btn-xs btn-primary glyphicon glyphicon-ok">Review('.$cc.'leftof/'.$count.')'.'</i> </a>';                                                
        return $btn;
          }
    if($data->valuecheck==2&&$cc!=0){                
//return Html::a('Disqualified', ['request/lview', 'id'=>$id], ['class'=>'btn btn-xs btn-primary']);
 $btn1 ='<a style="background-color:blue" data-method="post" data-confirm="Are you sure you want to Disqualified?" href="'.Url::to(["request/lview", 'id'=>$id]).'"><i class="btn btn-xs btn-primary glyphicon glyphicon-ok">Disqualified('.$cc.'leftof/'.$count.')'.'</i> </a>';                              
        return $btn1;
          }
   if($data->valuecheck==-1&&$cc!=0){                
//return Html::a('Rejected', ['request/lview', 'id'=>$id], ['class'=>'btn btn-xs btn-danger glyphicon glyphicon-remove']);
   $btn2='<a style="background-color:blue" data-method="post" data-confirm="Are you sure you want to Reject?" href="'.Url::to(["request/lview",'id'=>$id]).'"><i class="btn btn-xs btn-danger glyphicon glyphicon-remove">Rejected('.$cc.'/of'.$count.')'.'</i> </a>';                                          
        return $btn2;
          }
if($cc==0){
   return Html::a('Completed',['request/lview','id'=>$id], ['class'=>'btn btn-xs btn-success glyphicon glyphicon-ok']);
      }
else{
