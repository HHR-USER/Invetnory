...
'panel' => [
  ...
  if($curr==$end||$curr<$end){
     ...
     /*[
         ...
         'attribute'=> 'quantity',
          'headerOptions'=>["class"=>"text-success",'style'=>'font-color:red'],
         'value'=>function($model){
          $data=$model->id;
        $var =app\models\Consumablesmicro::find()->where(['id'=>$data])->one();
   $curr=strtotime(date('Y-m-d'));
  ...
  if($curr==$end||$curr<$end){
   ...
   $end=strtotime($var->expairedate); 
  if($curr==$end||$curr<$end){
       return $var->totalcost.$var->birrformat;
           }
           else{
            return "";
           }
  ...
  [ 'format'=>'raw',
            ...
            'value'=>function($model){
                  $id=$model->id;
       $var =app\models\Consumablesmicro::find()->where(['id'=>$id])->one();
       //$id=$var->id;
   $curr=strtotime(date('Y-m-d'));
   $end=strtotime($var->expairedate); 
  if($curr==$end||$curr<$end){
...
[
 ...
 $id=$data->id;
   ...
   $end=$var->expairedate; 
  if($curr==$end||$curr<$end){
return '<a data-method="post" data-confirm="Are you sure ?"'.Html::a('Undo',['consumablesmicro/undo','id' =>$id,],['class'=>'btn btn-xs btn-info fa fa-undo']);
       }}],],
    ]);?>
<?php Pjax::end(); ?>
</div>
<script type="text/javascript">          
  ...
  var href="<?=Yii::$app->request->baseUrl;?>/index.php/consumablesmicro/create2/"+"?id="+id+"&personnelid="+personnelid+"&quantity="+quantity+"&dt="+dt+"&office="+office;
