...
'panel' => [
 ...
 [
         ...
         'value'=>function($model){
          $data=$model->id;
        $var =app\models\Consumablesit::find()->where(['id'=>$data])->one();
   $curr=date('Y-m-d');
   $end=$var->expairedate; 
  if($curr==$end||$curr<$end||$model->expairedate==NULL){
        return $model['noi'];
           }
    else{
    return "  ";
  ...
  [         'format'=>'raw',
            ...
            'value'=>function($model){
                  $id=$model->id;
       $var =app\models\Consumablesit::find()->where(['id'=>$id])->one();
       //$id=$var->id;
   $curr=strtotime(date('Y-m-d'));
   $end=strtotime($var->expairedate); 
  if($curr==$end||$curr<$end||$model->expairedate==NULL){
...
return '<a data-method="post" data-confirm="Are you sure ?"'.Html::a('Undo',['consumablesit/undo','id' =>$id,],['class'=>'btn btn-xs btn-info fa fa-undo']);
       }}],
           ],
    ]);?>
<?php Pjax::end(); ?>
...
<script type="text/javascript">          
  ...
  if(personnelid ==''){
            ...
            alert("Please enter quantity you want to transfer");
        }
    else{
  var href="<?=Yii::$app->request->baseUrl;?>/index.php/consumablesit/create2/"+"?id="+id+"&personnelid="+personnelid+"&quantity="+quantity+"&dt="+dt+"&office="+office;
