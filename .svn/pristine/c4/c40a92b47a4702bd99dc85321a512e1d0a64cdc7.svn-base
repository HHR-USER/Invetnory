...
/**
 * AssetsitController implements the CRUD actions for Assetsit model.
 */
class AssetsitController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Lists all Assetsit models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new AssetsitSearch();
...
public function actionTransferi()
   {
    $searchModel=new AssetsitSearch();
    $searchModelOld=new CartassetsSearch();
    $dataProvider=$searchModel->search(Yii::$app->request->queryParams);
    $dataProviderOld=$searchModelOld->searchit(Yii::$app->request->queryParams);
 $dataProvider->sort->defaultOrder=['id'=>SORT_DESC];
  $dataProviderOld->sort->defaultOrder=['id'=>SORT_DESC];
$dataProvider->pagination->pageSize=1000;
    return $this->render('transferi', [
    ...
    /**
     * Displays a single Assetsit model.
     * @param integer $id
     * @return mixed
...
public function actionCreate2($id,$personnelid,$office,$returnables,$doreturnable)
     {
        $model=Assetsit::find()->where(['id'=>$id])->one(); 
  ...
  if($addm->save()) {
        $add->save();
        $log->save();
$sq="DELETE FROM assetsit WHERE id=$id";
        $query=Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['assetsit/transferi', 'id' => $model->id]);
     }
  else{
...
if($addp->office=="Pathology"||$addp->office=="IT"){
  if($addp->save()){
             $add->save();
             $log->save();
$sq="DELETE FROM assetsit WHERE id=$id";
...
public function actionCreate3($id)
    ...
    if($add->save()) {
        ...
        $query = Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['assetsit/transferi', 'id' => $add->id]);
       }
    \Yii::$app->session->setFlash('error','Not done');
           return $this->redirect(['assetsit/transferi', 'id' => $add->id]);
           }}
    /**
     * Updates an existing Assetsit model.
    ...
    /**
     * Deletes an existing Assetsit model.
    ...
    /**
     * Finds the Assetsit model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Assetsit the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Assetsit::findOne($id)) !== null) {
            return $model;
        }
