...
'panel' => [
       ...
       'before'=>Html::a('<i class="glyphicon glyphicon-plus"></i>Createrequest', ['createca'], ['class' => 'btn btn-primary','style'=>"float:right",'onClick'=>"popup()"]),
        'type'=>'info',
         ],
            'bootstrap' =>true,
        'hover'=>true,
    ...
    'columns'=>[
     [
        ...
        $last=substr($dicom,-6);
        if($req->dep=="Clinicala"&&$req->type=="None-consumable"){
        return $dicom;
        }
    else{
        return " ";
...
[
        ...
        'value'=>function ($data) {
         $req=app\models\Request::find()->where(['id'=>$data->id])->orderby('requestno')->one(); 
        $dicom=substr($req->requestno,0,1);
        $last=substr($dicom,-6);
    if($req->dep=="Clinicala"&&$req->type=="None-consumable"){
