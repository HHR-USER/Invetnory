...
<?=Highcharts::widget([
    'options'=>[
        ...
        "title"=>[
            "text"=>'Stocks consumption graphical this year report for each unit.',
        ],
        "subtitle"=>[
            "text"=>'This report details all transfer stock from the system being started'
        ],
        "xAxis" => [
             "categories"=>[
...
<?=Highcharts::widget([
                'options'=>[
                    ...
                    "xAxis"=>[
                        "categories"=>[
                            '<b>'.$monthlyreport['champ1']['month'].' '.(intval(date('Y')) ),
                            '<b>'.$monthlyreport['champ2']['month'].' '.(intval(date('Y')) ),
                            '<b>'.$monthlyreport['champ3']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ4']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ5']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ6']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ7']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ8']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ9']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ10']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ11']['month'].' '.(intval(date('Y'))),
                            '<b>'.$monthlyreport['champ12']['month'].' '.(intval(date('Y')))
                        ],
                    ],
                    "series"=>[[
                        "type"=>'column',
