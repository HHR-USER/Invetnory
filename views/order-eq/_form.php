...
<?php $form = ActiveForm::begin(['id' => 'dynamic-form']); ?>
  ...
  <?php
$type=['None-consumable'=>'None-consumable'];
  ?>
...
<div  class="container-items" width="50%">  
...
<?php foreach ($modelsAddress as $i => $modelAddress): ?>
  ...
  <td  width="15%">
  <?php                  
  $nonec="None Consumables";   
    //$pid=new app\models\Itemc(); 
      //$psg=\yii\helpers\ArrayHelper::map(app\models\Itemc::find()->where(['type'=>$nonec])->orderby('noi')->all(),'noi','noi');
     ?>      
<?=$form->field($modelAddress,"[{$i}]noi")->dropDownList(ArrayHelper::map(Itemc::find()->where(['type'=>$nonec])->orderby('noi')->all(),'noi','noi'),['prompt'=>'select items'])->label(false) ?> 
    </td>
...
<script type="text/javascript">          
   ...
   function popup(){
    ...
    function catagory()
     ...
     var href = "<?=Yii::$app->request->baseUrl;?>/index.php/order-eq/create1/"+"?type="+type+"&noi="+noi;
 ...
 <div class="modal fade" id="consentModal" tabindex="-1" role="dialog" aria-labelledby="consentModal">
  <div class="modal-dialog" role="document">
    ...
    <?php
$type=['None Consumables'=>'None Consumable'];
    ?> 
