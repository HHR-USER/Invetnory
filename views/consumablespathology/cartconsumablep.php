...
[
  ...
  [ 'format'=>'raw',
            ...
            'value'=>function($model){
                  $id=$model->id;
       $var=app\models\Consumablespathology::find()->where(['id'=>$id])->one();
   $curr=strtotime(date('Y-m-d'));
   $end=strtotime($var->expairedate); 
  if($curr==$end||$curr<$end){
...
'panel' => [
  ...
  [
   ...
   return  $var->noi;
     ...
     ['attribute'=> 'lot',
         'value'=>function($model){
          $data=$model->id;
        $var =app\models\Withdrow::find()->where(['id'=>$data])->one();
   $curr=strtotime(date('Y-m-d'));
   $end=strtotime($var->expairedate); 
  if($curr==$end||$curr<$end){
       return $var->lot;
 ...
 $id=$data->id;
   ...
   $end=$var->expairedate; 
  if($curr==$end||$curr<$end){
return '<a data-method="post" data-confirm="Are you sure ?"'.Html::a('Undo',['consumablespathology/undo','id' =>$id,],['class'=>'btn btn-xs btn-info fa fa-undo']);
       }}],
         ],
    ]);?>
<?php Pjax::end(); ?>
...
function followup()
    {
     ...
     var personnelid=$("#consumablespathology-personnelid").val();
     var dt=$("#consumablespathology-dt").val();
     var office=$("#consumablespathology-office").val();
  ...
  var href="<?=Yii::$app->request->baseUrl;?>/index.php/consumablespathology/create2/"+"?id="+id+"&personnelid="+personnelid+"&dt="+dt+"&quantity="+quantity+"&office="+office;
...
<div class="modal fade" id="consentModal" tabindex="-1" role="dialog" aria-labelledby="consentModal">
  <div class="modal-dialog" role="document">
    ...
    <div id="search1" style="display:inline;">  
          <?php
     $office=['Admin'=>'CHAMPS','Microlab'=>'Microlab','Clinical'=>'Clinical','IT'=>'IT(Information Tecknology)','SBS'=>'SBS','Pathology'=>'Pathology'];
        ?>
...
function getRelatedFields(personnelid){
    $.ajax({
        ...
        success:function (data) {
             $('#consumablespathology-firstname').val(data.firstname);
             $('#consumablespathology-personnelid').val(data.personnelid);
             $('#consumablespathology-office').val(data.office);
        },
...
?>
<div class="consumablespathology-form">
<?php
$idd=1.1;
