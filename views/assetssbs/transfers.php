...
[ 'attribute'=>'cost',
            'value'=>function($data){
                  $id=$data->id;
          $var=app\models\Assetssbs::find()->where(['id'=>$id])->one();
  ...
  [ 'format'=>'raw',
            ...
            'value'=>function($data){
                  $ids=$data->id;
          $model =app\models\Assetssbs::find()->where(['id'=>$ids])->one();
          $idd=$model->id;
...
[
      ...
      $var =app\models\Cartassets::find()->where(['id'=>$data->id])->one();
      $id=$var->id;
if($var->dep=="SBS"&&$var->returnables=='Y'){
...
var href="<?=Yii::$app->request->baseUrl;?>/index.php/assetssbs/create2/"+"?id="+id+"&personnelid="+personnelid+"&office="+office+"&returnables="+returnables+"&doreturnable="+doreturnable;
...
var id=$("#assetsid").val();
var href="<?=Yii::$app->request->baseUrl;?>/index.php/assetssbs/create3/"+"?id="+id;
        window.location.href=href;
          }
</script> 
