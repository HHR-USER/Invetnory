...
'panel' => [
        'heading'=>'<h3 class="panel-title"><i class="glyphicon glyphicon-circle-arrow-right"></i> Stock list</h3>',
            $mo=Assetsmicro::find()->where(['birrformat'=>'birr'])->one(),
        $moo=Assetsmicro::find()->where(['birrformat'=>'$'])->one(),
         $query=(new \yii\db\Query())->from('assetsmicro')->where(['birrformat'=>'birr']),
        $sum=$query->sum('cost'),
        $query1=(new \yii\db\Query())->from('assetsmicro')->where(['birrformat'=>'$']),
        $sum1=$query1->sum('cost'),
        //'after'=>Html::a('<i class="glyphicon glyphicon-circle-arrow-right"></i>Total cost bought in birr=>'.number_format($sum).$mo->birrformat."         ".'<i class="glyphicon glyphicon-circle-arrow-right"></i>Total cost bought in dollar=>'.number_format($sum1).$moo->birrformat,[''], ['class' => 'btn btn-success','style'=>"float:right"]),
    //'before'=>Html::a('<i class="glyphicon glyphicon-plus"></i>Creatasset', ['createas'], ['class' => 'btn btn-primary','style'=>"float:right"]),
      'type'=>'info',
         ],
        'bootstrap' =>true,
        'hover'=>true,
...
<script type="text/javascript">          
   ...
   function popup(id){
    ...
    function followup()
    {
     var fname=$("#assetsmicro-fname").val();
     var lname=$("#assetsmicro-lname").val();
     var mobile=$("#assetsmicro-mobile").val();
     var quantity=$("#assetsmicro-quantity").val();
     var office=$("#assetsmicro-office").val();
     var id=$("#massetsid").val();
    var href="<?=Yii::$app->request->baseUrl;?>/index.php/assetsmicro/create2/"+"?fname="+fname+"&lname="+lname+"&mobile="+mobile+"&quantity="+quantity+"&office="+office;
