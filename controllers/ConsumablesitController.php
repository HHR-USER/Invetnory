...
use app\models\ConsumablesitSearch;
...
/**
 * ConsumablesitController implements the CRUD actions for Consumablesit model.
 */
class ConsumablesitController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Lists all Consumablesit models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new ConsumablesitSearch();
...
public function actionCartconsumablei()
   {
    $searchModel = new ConsumablesitSearch();
    $searchModelOld = new WithdrowSearch();
    $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    $dataProviderOld = $searchModelOld->searchi(Yii::$app->request->queryParams);
 $dataProvider->sort->defaultOrder=['id'=>SORT_DESC];
$dataProvider->pagination->pageSize=1000;
    return $this->render('cartconsumablei', [
    ...
    /**
     * Displays a single Consumablesit model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new Consumablesit model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new Consumablesit();
...
public function actionCreate2($id,$personnelid,$quantity,$dt,$office)
         {
        $model=Consumablesit::find()->where(['id'=>$id])->one();
...
if($model->quantity==0){
              $addw->save();
              $log->save();
        $sq="DELETE FROM consumablesit WHERE id=$id";
        $query = Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['consumablesit/cartconsumablei', 'id' => $model->id]);
             }
else{  
    $command="UPDATE consumablesit SET quantity='$model->quantity' where id='$id'";
...
if($model->quantity==0){
        ...
        $query = Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['consumablesit/cartconsumablei', 'id' => $addp->id]);
             }
else{  
$command="UPDATE consumablesit SET quantity='$model->quantity' where id='$id'";
    Yii::$app->db->createCommand($command)->execute();
...
if($addu->office=="IT"&&$model->quantity>=0){
   if($addu->save()) {
if($model->quantity==0){
              $addw->save();
              $log->save();
...
public function actionUndo($id)
        ...
        $add=new Consumablesit();
        $reback=Consumablesit::find()->where(['id'=>$model->fki])->one();
    //Copying the data
        $add->noi=$model->noi;
        $add->packsize=$model->packsize;
        $add->unit=$model->unit;
...
if($reback==false){
    $add->save();
        ...
        $query=Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['consumablesit/cartconsumablei', 'id' => $add->id]);
    }
else{
    $q=$reback->quantity+$model->quantity;
     $cc="UPDATE consumablesit SET quantity='$q' where id='$model->fki'";
   ...
   \Yii::$app->session->setFlash('error','Please try again');
    ...
    /**
     * Updates an existing Consumablesit model.
    ...
    /**
     * Deletes an existing Consumablesit model.
    ...
    /**
     * Finds the Consumablesit model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Consumablesit the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Consumablesit::findOne($id)) !== null) {
            return $model;
        }
