...
if($var->dep=="Admin"){
  ...
  return "";
          ...
          [ 'attribute'=>'cost',
            'value'=>function($data){
                  $id=$data->id;
          $var=Assets::find()->where(['id'=>$id])->one();
...
[
      ...
      $var =app\models\Cartassets::find()->where(['id'=>$data->id])->one();
      $id=$var->id;
if($var->dep=="Admin"&&$var->returnables=='Y'){
...
var href="<?=Yii::$app->request->baseUrl;?>/index.php/assets/create2/"+"?id="+id+"&personnelid="+personnelid+"&office="+office+"&returnables="+returnables+"&doreturnable="+doreturnable;
