...
use app\models\AssetspathologySearch;
...
/**
 * AssetspathologyController implements the CRUD actions for Assetspathology model.
 */
class AssetspathologyController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Lists all Assetspathology models.
     * @return mixed
     */
public function actionIndex()
    {
        $searchModel = new AssetspathologySearch();
...
public function actionIndex1()
  ...
  public function actionTransferp()
   {
    $searchModel = new AssetspathologySearch();
...
public function actionCreate2($id,$personnelid,$office,$returnables,$doreturnable)
     {
      $model =Assetspathology::find()->where(['id'=>$id])->one();
    ...
    //Copying data
      ...
      $add->office=$office;
      $add->dep='Pathology';//Yii::$app->user->identity->Type;
  ...
  if($addm->save()) {
        $addp->save();
        $log->save();
$sq="DELETE FROM assetspathology WHERE id=$id";
        $query=Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
    return $this->redirect(['assetspathology/transferp', 'id' => $model->id]);  
     }
  else{
...
if($add->office=="Admin"||$add->office=="Pathology"){
  if($add->save()){
             $addp->save();
             $log->save();
     ...
     // $model = Yii::$app->db->createCommand($sq)->execute();
      ...
      $add->username=Yii::$app->user->identity->fname;
       $log=new Logtable();
        ...
        $log->action="Returned";
        $log->dateentered=date('Y-m-d');
    if($add->dep=="Pathology"){
    if($add->save()) {
        $log->save();
        $sq="DELETE FROM cartassets WHERE id=$id";
        $query = Yii::$app->db->createCommand($sq)->execute();
           \Yii::$app->session->setFlash('success','Success');
           return $this->redirect(['assetspathology/transferp', 'id' => $add->id]);
       }}
